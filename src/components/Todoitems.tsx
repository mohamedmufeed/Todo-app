import tick from "../assets/tick.png"
import not_tick from "../assets/not_tick.png"
import delete_icon from "../assets/delete.png"
import edit_icon from "../assets/edit-246-128.png"
import React from "react"
import Todo from "./Todo"


interface TodoItemsProps {
    id: Date;
    text: string | undefined;
    isComplete: boolean;
    deleteTodo: (id: Date) => void;
    toggle: (id: Date) => void;
    editTodo:(id:Todo)=>void
}



const Todoitems: React.FC<TodoItemsProps> = ({ text, id, isComplete, deleteTodo, toggle,editTodo }) => {
    return (
        <div className='flex items-center my-3 gap-2'>

            <div onClick={() => { toggle(id) }} className='flex flex-1 items-center cursor-pointer'>
                <img className="w-7 " src={isComplete ? tick : not_tick} alt="" />
                <p className={`text-slate-700 ml-4 text-[17px] decoration-slate-500 ${isComplete ? "line-through " : ""}`}>{text}</p>
            </div>
            <img src={edit_icon} onClick={()=>{editTodo({id,isComplete,text})}} className=" w-5 cursor-pointer" alt="" />
            <img onClick={() => { deleteTodo(id) }} className="w-3.5 cursor-pointer" src={delete_icon} alt="" />
        </div>
    )
}

export default Todoitems 